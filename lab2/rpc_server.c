...
static void handle_rpc_request(int connfd) {
    uint32_t proc_id_n;
    if (readn(connfd, &proc_id_n, sizeof(proc_id_n)) != sizeof(proc_id_n)) {
    ...
    printf("Receiving file '%s' (%llu bytes) via RPC\n",
    ...
    if (remaining == 0) {
        ...
        fprintf(stderr, "File incomplete (missing %llu bytes)\n",
                (unsigned long long)remaining);
    }

    int32_t status_n = htonl(status);
    if (writen(connfd, &status_n, sizeof(status_n)) != sizeof(status_n)) {
        fprintf(stderr, "Failed to send status back to client\n");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        printf("New RPC connection accepted.\n");
        handle_rpc_request(connfd);
        close(connfd);
