...
static int rpc_send_file(int sockfd, const char *path) {
    ...
    printf("RPC: sending file '%s' (%llu bytes)\n",
           name, (unsigned long long)size);

    uint32_t proc_id_n = htonl(PROC_SEND_FILE);
    if (writen(sockfd, &proc_id_n, sizeof(proc_id_n)) != sizeof(proc_id_n)) {
        fprintf(stderr, "Failed to send procedure id\n");
        close(fd);
        return -1;
    }
    ...
    int32_t status_n;
    if (readn(sockfd, &status_n, sizeof(status_n)) != sizeof(status_n)) {
        fprintf(stderr, "Failed to receive status from server\n");
        return -1;
    }
    int32_t status = ntohl(status_n);
    if (status == 0) {
        printf("RPC: file transfer completed successfully.\n");
    } else {
        printf("RPC: file transfer failed with status %d.\n", status);
    }
    return status;
}
...
int main(int argc, char *argv[]) {
    ...
    int rc = rpc_send_file(sockfd, path);
